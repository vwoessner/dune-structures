...
int main(int argc, char** argv)
{
  ...
  using GridProvider = Dune::BlockLab::StructuredSimplexGridProvider<Dune::UGGrid<3>>;
  ...
  auto vectors = std::make_tuple(
    std::make_tuple("Displacement Field", "P1 Lagrange Element", [](auto gp)
      {
    ...
    std::make_tuple("Body Force", "P1 Lagrange Element", [](auto gp)
      {
        using GridProvider = typename decltype(gp)::element_type;
        auto leaf = std::make_shared<Dune::BlockLab::PkFemVectorProvider<GridProvider, 1>>(gp);
        return Dune::BlockLab::fieldProvider<GridProvider::Grid::dimension>(leaf);
      }),
    std::make_tuple("Traction Force", "P1 Lagrange Element", [](auto gp)
      {
        using GridProvider = typename decltype(gp)::element_type;
        auto leaf = std::make_shared<Dune::BlockLab::PkFemVectorProvider<GridProvider, 1>>(gp);
        return Dune::BlockLab::fieldProvider<GridProvider::Grid::dimension>(leaf);
  ...
  app.addHelpMessage();
  app.setTitle("Structured 3D Cube Grid - Linear Elements");
