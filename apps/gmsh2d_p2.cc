...
main(int argc, char** argv)
{
  ...
  using GridProvider = Dune::BlockLab::GMSHGridProvider<Dune::UGGrid<2>>;
  auto grid = [](const auto& c) { return std::make_shared<GridProvider>(c); };
  ...
  auto vectors = std::make_tuple(
    std::make_tuple(
      ...
      [](auto gp) {
        ...
        auto leaf = std::make_shared<
          Dune::BlockLab::PkFemVectorProvider<GridProvider, 2>>(gp);
        return Dune::BlockLab::fieldProvider<GridProvider::Grid::dimension>(
          leaf);
      }),
    std::make_tuple(
      "Body Force",
    ...
    std::make_tuple("Traction Force", "P2 Lagrange Element", [](auto gp) {
      using GridProvider = typename decltype(gp)::element_type;
      auto leaf =
        std::make_shared<Dune::BlockLab::PkFemVectorProvider<GridProvider, 2>>(
          gp);
  ...
  Dune::BlockLab::BlockLabApp app(
    init, grid, vectors, registerStructuresBlocks, ParameterTuple{});

  app.addDefaultRunner();
  app.addFrontendExporter();
  app.addHelpMessage();
  app.setTitle("Unstructured 2D GMSH Grid - Quadratic Elements");
