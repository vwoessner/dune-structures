...
template<int dim>
YAML::Node
write_gmsh_config(const YAML::Node& config)
{
  ...
  auto command =
    std::string(VIRTUALENV_WRAPPER_DIR)
    + "/run-in-dune-env generate_cell_mesh gridconfig.yml >meshgen.log"s;
  std::system(command.c_str());

  // And return a config that is understood by dune-blocklab's GMSH grid
  // provider
  YAML::Node ret;
...
public:
  StructuresGridProvider(const YAML::Node& config)
    : Dune::BlockLab::GMSHGridProvider<Dune::UGGrid<dim>>(
      impl::write_gmsh_config<dim>(config))
  {
  }

  static std::vector<std::string> blockData()
  {
    return { "title: Structures Cell Grid                          \n"
             "category: grids                                      \n"
             "schema:                                              \n"
             "  cytoplasm:                                         \n"
             "    type: dict                                       \n"
