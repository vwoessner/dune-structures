...
// A comparison object to define a total ordering on Dune::FieldVectors for the
// purpose of defining sorting criteria for cells based on center coordinates.
struct LexicographicFieldVectorComparator
{
  template<typename T, int dim>
  bool operator()(const Dune::FieldVector<T, dim>& a,
                  const Dune::FieldVector<T, dim>& b) const
  {
    ...
    // If we ever make it here, a and b are fuzzy equal. In that case we do not
...
auto
reorder_impl(Grid&& grid, ES&& es, Physical&& physical, Compare&& compare)
{
  ...
  {
    std::vector<
      typename std::decay_t<ES>::Traits::Element::Geometry::GlobalCoordinate>
      vertex_positions(es.size(dim));
             ...
             const Dune::ParameterTree& params)
{
  ...
  if (strategy == "lefttoright")
    return reorder_impl(
      grid, es, physical, [grid](const auto& seed1, const auto& seed2) {
