...
public:
  ...
  virtual void evaluate(const Entity&,
                        const Coord&,
                        Dune::FieldMatrix<T, dim, dim>&) const = 0;
};

template<typename GV, typename T>
class NoPrestress : public MaterialPrestressBase<GV, T>
{
public:
  using Entity = typename GV::template Codim<0>::Entity;
  using Coord = typename GV::template Codim<0>::Geometry::LocalCoordinate;
...
template<typename GV, typename T>
class IsotropicPrestress : public MaterialPrestressBase<GV, T>
{
public:
  using Entity = typename GV::template Codim<0>::Entity;
  using Coord = typename GV::template Codim<0>::Geometry::LocalCoordinate;
...
template<typename GV, typename T>
class StraightFibrePrestress : public MaterialPrestressBase<GV, T>
{
public:
  using Entity = typename GV::template Codim<0>::Entity;
  using Coord = typename GV::template Codim<0>::Geometry::LocalCoordinate;
...
class CurvedFibrePrestressImpl : public NoPrestress<GV, T>
{
public:
...
template<typename GV, typename T>
class CurvedFibrePrestressImpl<GV, T, 3> : public MaterialPrestressBase<GV, T>
{
public:
  using Entity = typename GV::template Codim<0>::Entity;
  using Coord = typename GV::template Codim<0>::Geometry::LocalCoordinate;

  CurvedFibrePrestressImpl(const YAML::Node& param,
    ...
    , subsampling(param["subsampling"].as<int>(100))
  {
    ...
    if (fibreconfig["shape"].as<std::string>() != "overnucleus")
      DUNE_THROW(MaterialError,
                 "CurvedFibrePrestress only supports overnucleus fibres!");

    // Extracting control points from config. This makes quite some assumptions
    // of course.
    ...
    control_points[0][1] =
      Dune::FieldVector<T, 3>{ { (1.0 - slope) * start[0] + slope * middle[0],
    ...
    control_points[1][1] =
      Dune::FieldVector<T, 3>{ { (1.0 - slope) * middle[0] + slope * end[0],
  ...
  virtual void evaluate(const Entity& e,
                        const Coord& x,
                        Dune::FieldMatrix<T, 3, 3>& m) const override
  {
...
private:
  ...
  {
    return n
           * (bernstein_polynomial(n - 1, i - 1, x)
              - bernstein_polynomial(n - 1, i, x));
  }
  ...
  T minimize_across_curve(const Dune::FieldVector<T, 3>& point,
                          int cpindex) const
  {
    T h = 1.0 / sampling;
    ...
    auto cmp = [this, point, cpindex](auto a, auto b) {
      return this->distance(point, cpindex, a)
             < this->distance(point, cpindex, b);
    };

    T close =
      *std::min_element(sample_points.begin(), sample_points.end(), cmp);
    ...
    return *std::min_element(sample_points.begin(), sample_points.end(), cmp);
  }
...
template<typename GV, typename T>
std::shared_ptr<MaterialPrestressBase<GV, T>>
construct_prestress(const YAML::Node& params, const YAML::Node& rootparams)
{
  std::string type;
