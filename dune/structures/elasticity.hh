...
using OperatorSwitch = OperatorSwitchImpl<
  GFS,
  dim,
  typename GFS::template Child<0>::Type::Traits::FiniteElementMap,
  FGFS,
  TGFS>;
...
struct OperatorSwitchImpl<
  GFS,
  3,
  Dune::PDELab::PkLocalFiniteElementMap<ES, double, R, 1>,
  FGFS,
  TGFS>
{
...
class ElasticityOperatorBlock : public Dune::BlockLab::DisabledBlock<P, V, i>
{
public:
...
public:
  ...
  using LocalOperator =
    typename OperatorSwitch<typename Traits::GridFunctionSpace,
                            dim,
                            FGFS,
                            TGFS>::Elasticity;
  ...
  virtual void setup() override
  {
    ...
    auto lop = std::make_shared<LocalOperator>(
      *gfs, *gfs, Dune::ParameterTree(), material);
  ...
  static std::vector<std::string> blockData()
  {
    ...
    data.push_back("title: Elasticity Operator                          \n"
                   "category: operators                                 \n");
    return data;
  }
};
...
public:
  ...
  virtual void setup() override
  {
    ...
    auto vector = this->solver->template getVector<i>();
    auto gfs = vector->gridFunctionSpaceStorage();
    auto lop = std::make_shared<TemporalLocalOperator>(
      *gfs, *gfs, Dune::ParameterTree());
  ...
  {
    auto data = Dune::BlockLab::BlockBase<P, V, i>::blockData();
    data.push_back("title: Elasticity Mass Operator                     \n"
                   "category: operators                                 \n");
    return data;
  }
};
