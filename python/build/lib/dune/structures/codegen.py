...
from dune.codegen.pdelab.localoperator import (
...
class UFLPhysicalParameter(Coefficient):
    ...
    def visit(self, visitor):
        ...
        qp = tuple(
            maybe_wrap_subscript(visitor.to_cell(visitor.quadrature_position()), i)
...
class UFLPrestress(Coefficient):
    ...
    def visit(self, visitor):
        ...
        temporary_variable(name, shape=(dim, dim), managed=False, shape_impl=("fm",))
        material_class = name_material_class()

        instruction(
            code="{}->prestress({}, {}, {});".format(
                material_class, str(cell), str(visitor.quadrature_position()), name
            ),
...
def dune_structures_function_mangler(knl, func, arg_dtypes):
    if isinstance(func, LoopyPhysicalParameter):
        return lp.CallMangleInfo(
            ...
            (lp.types.to_loopy_type(np.dtype(str)), lp.types.to_loopy_type(np.int32))
    ...
    if isinstance(func, LoopyEntity):
        return lp.CallMangleInfo(
            func.name, (lp.types.to_loopy_type(np.dtype(str)),), ()
        )
...
def define_material_class(name):
    ...
    _type = "std::shared_ptr<ElasticMaterialBase<typename {}::Traits::EntitySet, {}>>".format(
        lop_template_ansatz_gfs(), type_floatingpoint()
    )
...
def setter_material_class(name):
    ...
    return [
        "void setMaterial({} {}_)".format(_type, name),
        "{",
        "  {} = {}_;".format(name, name),
        "}",
    ]
...
def notemplate_construct_signature(types, args, name):
    ...
    func = "void {}({}) const override".format(
        name,
        ", ".join(
            "{}{}& {}".format("const " if c else "", t, a)
            for t, (c, a) in zip(types, args)
        ),
    )
    return [func]
...
def baseclass_hook():
    ...
    base_class(
        "Dune::BlockLab::AbstractLocalOperatorInterface<{}, {}>".format(gfsu, gfsv),
        classtag="operator",
    )
