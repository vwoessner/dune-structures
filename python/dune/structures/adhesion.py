...
def deshpande_adhesion(form, where):
    ...
    high_concentration = params.total_integrin_concentration / (
        exp(
            -(params.internal_energy_difference + potential - sumforce)
            / (params.temperature * params.boltzmann_constant)
        )
        + 1
    )
